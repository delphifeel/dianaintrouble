...
const SWITCH_CORNER_TIMEOUT: f32 = 0.1;
...
fn set_new_pos(self: *Self, player_center: rl.Vector2, frame_time: f32) void {
    ...
    if (self.time_passed < SWITCH_CORNER_TIMEOUT) {
        self.transform.x = player_center.x + OFFSET_FROM_CENTER - SIZE / 2;
        self.transform.y = player_center.y - OFFSET_FROM_CENTER - SIZE / 2;
    } else if (self.time_passed < SWITCH_CORNER_TIMEOUT * 2) {
        self.transform.x = player_center.x + OFFSET_FROM_CENTER - SIZE / 2;
        self.transform.y = player_center.y + OFFSET_FROM_CENTER - SIZE / 2;
    } else if (self.time_passed < SWITCH_CORNER_TIMEOUT * 3) {
        self.transform.x = player_center.x - OFFSET_FROM_CENTER - SIZE / 2;
        self.transform.y = player_center.y + OFFSET_FROM_CENTER - SIZE / 2;
    } else if (self.time_passed < SWITCH_CORNER_TIMEOUT * 4) {
        self.transform.x = player_center.x - OFFSET_FROM_CENTER - SIZE / 2;
        self.transform.y = player_center.y - OFFSET_FROM_CENTER - SIZE / 2;
    }
}
